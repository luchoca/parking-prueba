...
1.Descomprimir archivo que se envia por mail o sino:
...
4. Abre un navegador web y accede al proyecto en la siguiente dirección:
   ...
   Para asi poder iniciar la BD y la tabla de vehiculos.
   Debera aprecer este string Base de datos creada o ya existente.Tabla 'vehiculos' creada o ya existente.

   ```
   http://localhost:8888/prueba-luciano.castro/back/backend.php 
   ```

5
. Interactúa con la interfaz de usuario para registrar vehículos y asignarles lugares en el estacionamiento.	
	-Esto se puede hacer accediendo a el index.html para interactuar, puede hacerlo desde el IDE utilizando el LiveServer.
